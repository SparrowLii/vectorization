...
use rustc_middle::ty::{self, Const, TyCtxt, TypeAndMut};
...
#[derive(Debug, Clone)]
enum OriginStmt<'tcx> {
    CondiStorage(Location),
    CondiGet(Place<'tcx>, Rvalue<'tcx>),
    CondiSet(Place<'tcx>, Rvalue<'tcx>),
    ...
    BreakTerminator(Location),
    SetCondiTerminator(Location),
    GetCondiTerminator(Location),

    GotoTerminator(Location),
    EndLoopTerminator(Location),
    TempFuncTerminator(Location, Place<'tcx>),

    OtherTerminator(Location),
    StoreAfter(Place<'tcx>),
}
...
impl<'tcx> VectorResolver<'tcx> {
                ...
                match &statement.kind {
                    StorageLive(local) | StorageDead(local) => {
                        ...
                        match self.r.locals_use[*local] {
                            Condition => self.r.loop_stmts.push(CondiStorage(location)),
                    ...
                    Assign(assign) => {
                        ...
                        match (left_use, right_use) {
                            (Condition, _) => {
                                ...
                                self.r.loop_stmts.push(CondiSet(*place, rvalue.clone()))
                            }
                            (InLoop, Condition) => {
                                left_locals.union(&right_locals);
                                self.r.loop_stmts.push(CondiGet(*place, rvalue.clone()));
                            }
                ...
                match &terminator.kind {
                    SwitchInt { discr, switch_ty: _, targets: _ } => {
                        ...
                        if self.r.break_blocks.contains(&location.block) {
                            self.r.loop_stmts.push(BreakTerminator(location));
                        } else {
                    ...
                    Call { func, args, destination, cleanup: _, from_hir_call: _, fn_span: _ } => {
                        ...
                        match (return_use, arg_use) {
                            (Condition, _) => self.r.loop_stmts.push(SetCondiTerminator(location)),
                            (InLoop, Condition) => {
                                self.r.loop_stmts.push(GetCondiTerminator(location))
                            }
                            (InLoop, InLoop) => {
                    ...
                    _ => {
                        unimplemented!("unsupported terminator in {:?}", location)
                    }
                }
            }
        ...
        while changed {
            ...
            for stmt in &self.loop_stmts {
                match stmt {
                    ...
                    CondiSet(place, rvalue)
                    | CondiGet(place, rvalue)
    ...
    fn generate_blocks<'r>(&'r mut self, tcx: TyCtxt<'tcx>, body: &mut Body<'tcx>) {
        ...
        let (condi_stmts, vect_stmts) = (inner.to_vec(), vect.to_vec());
            ...
            match stmt {
                ...
                AfterSet(place, rvalue, _bitset, _) => match rvalue {
                    Rvalue::BinaryOp(binop, ops) | Rvalue::CheckedBinaryOp(binop, ops) => {
                        // This statement can be vectorized if the value of the variable
                        // is not read in other statements in the loop.
                        let after = place.local;
                        let (op1, op2) = ops.as_ref();
                        let mut after_read = false;
                        for i in 0..vect_stmts.len() {
                            match &vect_stmts[i] {
                                AfterSet(place2, rvalue, right_locals, _)
                                | AfterGet(place2, rvalue, right_locals, _) => {
                                    if right_locals.contains(after) {
                                        // Changing your own value does not block
                                        // vectorization.
                                        if place2.local == after {
                                            continue;
                                        }
                ...
                TempFuncTerminator(location, _) => {
                    ...
                    } = body[location.block].terminator().kind.clone()
                    {
                        ...
                        // Whether it's a specific instruction call
                        if func_str == sym::sqrtf32 {
    ...
    fn resolve_temp_to_vector<'r>(
        ...
        tcx: TyCtxt<'tcx>,
        body: &mut Body<'tcx>,
        condi_stmts: &Vec<OriginStmt<'tcx>>,
    ...
    fn generate_condi_section<'r>(
        ...
        self.inner_set0(tcx, body, outer_loop, inner, source_info);
        ...
        let temp2 = self.inner_to_temp(tcx, body, inner_loop, inner, source_info);
        ...
        for stmt in condi_stmts {
            match stmt {
                CondiStorage(location) | TempStorage(location) => {
                ...
                BreakTerminator(location) => {
                    ...
                    if let SwitchInt { discr, switch_ty, targets: mut new_targets } =
                        term.kind.clone()
                    {
                        let next = body.basic_blocks_mut().push(BasicBlockData::new(None));
                        ...
                        body[block].terminator = Some(Terminator {
                            source_info,
                            kind: SwitchInt { discr: discr, switch_ty, targets: new_targets },
                        });
        ...
        self.inner_add1(tcx, body, block, inner, source_info);
    ...
    fn new_inner_loop<'r>(
        ...
        body: &mut Body<'tcx>,
        current: BasicBlock,
        stmts: Vec<OriginStmt<'tcx>>,
        inner: Local,
    ) -> BasicBlock {
        let source_info = SourceInfo { span: DUMMY_SP, scope: OUTERMOST_SOURCE_SCOPE };
        self.inner_set0(tcx, body, current, inner, source_info);
        ...
        let temp2 = self.inner_to_temp(tcx, body, loop_start, inner, source_info);
        ...
        self.inner_add1(tcx, body, current, inner, source_info);
        body[current].terminator =
            Some(Terminator { source_info, kind: Goto { target: loop_start } });
                    ...
                    BinOpAfterSet(place, bin_op, op1, op2) => {
                        ...
                        match (self.locals_use.get(l1), self.locals_use.get(l2)) {
                            (Some(AfterLoop), Some(InLoop)) => {
                                if l1 != place.local {
                                    unimplemented!("not self-assign vector stmt")
                                }
                            ...
                            (Some(InLoop), Some(InLoop)) => {
                                ...
                                current = next;
                                inner_stmts.push(StoreAfter(place));
                            }
    ...
    fn generate_remain_section<'r>(
        ...
        for stmt in pre_remain_stmts {
            match stmt {
                ...
                RemainStmt::SetAfterReduce(place, bin_op) => {
                    let temp =
                        body.local_decls.push(LocalDecl::new(place.ty(body, tcx).ty, DUMMY_SP));
                    ...
                    body[remain_init].terminator = Some(Terminator {
                        ...
                        kind: VectorFunc {
                            func,
                            args,
                            destination: Some((Place::from(temp), next)),
                        },
                    });
                    remain_init = next;
                    body[remain_init].statements.push(Statement {
                        ...
                        kind: Assign(Box::new((
                            ...
                            Rvalue::BinaryOp(
                                bin_op,
                                Box::new((Operand::Copy(place), Operand::Move(Place::from(temp)))),
                            ),
                        ))),
                    })
                }
        ...
        self.inner_set0(tcx, body, remain_init, inside, source_info);
        ...
        for stmt in vect_stmts {
            match stmt {
                ...
                EndLoopTerminator(_location) => {
                    self.inner_add1(tcx, body, block, inside, source_info);
                    body[block].terminator =
                        Some(Terminator { source_info, kind: Goto { target: remain_start } });
                }
                OtherTerminator(location) => {
